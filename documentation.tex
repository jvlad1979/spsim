...
\usetikzlibrary{shapes.geometric, positioning} % Added TikZ libraries
...
\begin{figure}[h!]
    ...
    \begin{tikzpicture}[scale=0.8]
        % Define layers
        \fill[gray!20] (0,0) rectangle (6,-1.5); % Substrate
        \node at (3,-0.75) {Substrate (GaAs)};
        \fill[blue!10] (0,0) rectangle (6,1.5); % Barrier
        \node at (3,0.75) {Barrier (AlGaAs)};
        % 2DEG layer - represent as a thin shaded region
        \fill[blue!50!black, opacity=0.5] (0,1.3) rectangle (6,1.4);
        \node[above] at (3,1.4) {2DEG};
        % Surface
        \draw (0,1.5) -- (6,1.5);
        \node[above] at (3,1.5) {Surface};
        ...
        % Indicate potential influence with dashed lines
        \draw[->, dashed] (1.75,1.6) -- (1.75,1.45);
        \draw[->, dashed] (4.25,1.6) -- (4.25,1.45);

        % Add labels for confinement directions
        \node[left] at (0,0.75) {z-confinement};
        \draw[<->] (-0.5,0) -- (-0.5,1.5);
        \node[below] at (3,-1.5) {x, y-confinement};
        \draw[<->] (0,-2) -- (6,-2);
    \end{tikzpicture}
...
\begin{figure}[h!]
    ...
    \begin{tikzpicture}[scale=0.8]
        ...
        % Gate std dev vis ~ 20nm. Ellipse radius ~ 2*std_dev_vis = 40nm. Scaled: 1.6 units.

        % Gate Ellipses (center, x-radius, y-radius)
        % Plungers (P1, P2) - typically negative voltage, create wells
        \draw[blue, dashed, thick] (2.1, 2.0) ellipse (0.8 and 0.8); \node[above=2pt] at (2.1, 2.0) {P1};
        \draw[cyan, dashed, thick] (3.9, 2.0) ellipse (0.8 and 0.8); \node[above=2pt] at (3.9, 2.0) {P2};

        % Barriers (B1, B2, B3) - typically positive voltage, create barriers
        \draw[red, dotted, thick] (0.9, 2.0) ellipse (0.8 and 0.8); \node[above=2pt] at (0.9, 2.0) {B1};
        \draw[magenta, dotted, thick] (3.0, 2.0) ellipse (0.8 and 0.8); \node[above=2pt] at (3.0, 2.0) {B2};
        \draw[orange, dotted, thick] (5.1, 2.0) ellipse (0.8 and 0.8); \node[above=2pt] at (5.1, 2.0) {B3};

        % Indicate approximate dot locations (potential minima)
        \node[draw, circle, inner sep=2pt, fill=blue!20, thick] at (2.5, 2.0) {}; \node[below=2pt] at (2.5, 2.0) {Dot 1};
        \node[draw, circle, inner sep=2pt, fill=cyan!20, thick] at (3.5, 2.0) {}; \node[below=2pt] at (3.5, 2.0) {Dot 2};

        % Add scale bar
        \draw[<->] (0, -0.5) -- (6, -0.5) node[midway, below] {Lx (150 nm)};
        \draw[<->] (-0.5, 0) -- (-0.5, 4) node[midway, left] {Ly (100 nm)};

        % Add a simple legend
        \node[below right, inner sep=5pt, draw, thick] at (6,0) {
            \begin{tabular}{ll}
                \tikz{\draw[blue, dashed, thick] (0,0) -- (0.5,0);} & Plunger Gate \\
                \tikz{\draw[red, dotted, thick] (0,0) -- (0.5,0);} & Barrier Gate \\
                \tikz{\node[draw, circle, inner sep=1pt, fill=blue!20, thick] {};} & Approx. Dot Location \\
            \end{tabular}
        };
