...
def get_external_potential(x, voltages):
    ...
    potential += gaussian_potential(
        p1_center, gate_std_dev, voltages.get("P1", 0.0) * e
    )
    potential += gaussian_potential(
        p2_center, gate_std_dev, voltages.get("P2", 0.0) * e
    )
    ...
    potential += gaussian_potential(
        b1_center, gate_std_dev, voltages.get("B1", 0.0) * e
    )
    potential += gaussian_potential(
        b2_center, gate_std_dev, voltages.get("B2", 0.0) * e
    )
    potential += gaussian_potential(
        b3_center, gate_std_dev, voltages.get("B3", 0.0) * e
    )
...
def self_consistent_solver(voltages, fermi_level, max_iter=100, tol=1e-6, mixing=0.1):
    ...
    for i in range(max_iter):
        ...
        new_charge_density = calculate_charge_density(
            eigenvalues, eigenvectors, fermi_level
        )
        ...
        potential_diff = (
            np.linalg.norm(new_electrostatic_potential_V - electrostatic_potential_V)
            * dx
        )
        ...
        charge_density = calculate_charge_density(
            eigenvalues, eigenvectors, fermi_level
        )  # Recalculate based on current states
    ...
    final_charge_density = calculate_charge_density(
        final_eigenvalues, final_eigenvectors, fermi_level
    )
...
if __name__ == "__main__":
    ...
    fermi_level_J = (
        np.min(ext_pot) + 0.05 * e
    )  # Example: 50 meV above min potential (in Joules)
    ...
    if total_potential is not None:
        ...
        plt.plot(
            x * 1e9,
            get_external_potential(x, applied_voltages) / e,
            "--",
            label="External Potential (eV)",
        )
        plt.axhline(
            fermi_level_J / e,
            color="r",
        ...
        plt.plot(
            x * 1e9, charge_density_per_nm, label="Electron Density (electrons/nm)"
        )
        ...
        if eigenvalues.size > 0:
            ...
            density_min, density_max = (
                np.min(charge_density_per_nm),
                np.max(charge_density_per_nm),
            )
            density_range = max(
                density_max - density_min, 1e-9
            )  # Avoid division by zero if density is flat
            scale = (
                density_range * 2.0
            )  # Scale factor for wavefunctions relative to density range
            ...
            for i in range(min(5, len(eigenvalues))):  # Plot up to 5 lowest states
                ...
                psi_scaled *= (
                    scale / np.max(psi_scaled) if np.max(psi_scaled) > 1e-9 else 1.0
                )
                psi_plot = (
                    eigenvalues[i] / e
                ) + psi_scaled  # Shift baseline to energy level
                (line,) = plt.plot(
                    x * 1e9, psi_plot, label=f"E{i} ({eigenvalues[i] / e:.3f} eV)"
                )
                plot_lines.append(line)

        plt.xlabel("Position (nm)")
        plt.ylabel("Electron Density / Scaled $|\phi|^2$")
        ...
        plt.ylim(
            min_y_val - 0.1 * abs(min_y_val), max_y_val + 0.1 * abs(max_y_val)
        )  # Add 10% margin
